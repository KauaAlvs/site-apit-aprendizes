...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ------------------------------------- */
    /* 5. LÓGICA DAS ABAS (PÁGINA PROGRAMAS) */
    /* ------------------------------------- */
    const tabButtons = document.querySelectorAll('.publico-tab-btn');
    const tabPanels = document.querySelectorAll('.publico-tab-panel');
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            document.querySelector(`.publico-tab-panel[data-tab="${targetTab}"]`).classList.add('active');

            // Re-observa elementos de animação dentro da nova aba ativa
            const newAnimatedElements = document.querySelectorAll(`.publico-tab-panel[data-tab="${targetTab}"] .animate-on-scroll`);
            newAnimatedElements.forEach(el => {
                el.classList.remove('is-visible'); // Reseta a animação
                scrollObserver.observe(el); // Observa novamente
            });
        });
    });
